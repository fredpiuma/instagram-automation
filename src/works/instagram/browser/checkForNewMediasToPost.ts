...
export default async function checkForNewMediasToPost(avatar: AvatarInstagram): Promise<AvatarInstagram> {
  const work = avatar.getWork(Works.instagramBrowserCheckForNewMediasToPost)
  ...
  try {
    ...
    while ((instagramMediaSchedule = instagramMediaSchedules.shift())) {
      ...
      if (instagramMediaSchedule.instagram_media_id !== null && instagramMediaSchedule.instagram_media_id.length > 1) {
        instagramMedia = InstagramMedia.fromInstagramRawJson(
          await InstagramBrowser.getMediaInfoByMediaId(avatar, instagramMediaSchedule.instagram_media_id)
        )

        monitor = await InstagramBrowser.getMonitorInstagramAccountByPerfilInstagramId(instagramMedia.user_id)

        avatarInstagramRepostMedia = monitor
          ? await InstagramBrowser.getAvatarInstagramRepostMedia(avatar.id, monitor.id)
          : undefined
      ...
      if (instagramMediaSchedule.instagram_media_library_id !== null) {
        const lastPostedFromSameLibraryItems: InstagramMediaSchedule[] =
          await InstagramBrowser.getInstagramMediaSchedulePostedFromSameLibraryItem(
        ...
        if (lastPostedFromSameLibraryItems.length > 0) {
          for (const lastPostedFromSameLibraryItem of lastPostedFromSameLibraryItems) {
            if (lastPostedFromSameLibraryItem.posted_instagram_media_id !== null) {
              ...
              await InstagramBrowser.deleteMedia(avatar, lastMedia)

              await InstagramBrowser.updateSeguroFilter(
      ...
      switch (instagramMediaSchedule.type) {
        ...
        case 'clips':
        case 'story':
          publishResult = await InstagramBrowser.postOnFeed(
            avatar,
            {
      ...
      if (avatarInstagramRepostMedia && avatarInstagramRepostMedia.delete_original.toString() === '1') {
        await InstagramBrowser.deleteMedia(avatar, instagramMedia)
      }
      ...
      await InstagramBrowser.setInstagramMediaScheduleAsPosted(instagramMediaSchedule)
    ...
    if (
      ...
      await InstagramBrowser.deleteSeguro('Instagram_media_schedule', {
        id: instagramMediaSchedule.id
      })
      ...
      await InstagramBrowser.deleteSeguro('Instagram_media', {
