...
export class PostsInsightsFeed extends Feed<
  PostsInsightsFeedResponseRootObject,
  PostsInsightsFeedResponseEdgesItem
> {
  ...
  async request(): Promise<PostsInsightsFeedResponseRootObject> {
    const body =
      await this.client.ads.graphQL<PostsInsightsFeedResponseRootObject>({
  ...
  protected set state(response: PostsInsightsFeedResponseRootObject) {
    const { end_cursor, has_next_page } =
      response.data.user.business_manager.top_posts_unit.top_posts.page_info
