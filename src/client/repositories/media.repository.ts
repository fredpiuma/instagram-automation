...
export class MediaRepository extends Repository {
  ...
  public async delete({
    ...
    await this.client.request.sendUnauthenticatedOptionsRequest(
      {
        url: `/api/v1/web/create/${mediaId}/delete/`,
        method: 'OPTIONS',
        headers: {
  ...
  private async likeAction(options: MediaLikeOrUnlikeOptions) {
    const { body } = await this.client.request.send({
      url: `/web/likes/${options.mediaId}/${options.action}/`,
      method: 'POST',
    })
    return body
  }
  ...
  public async configure(options: MediaConfigureTimelineOptions): Promise<MediaRepositoryConfigureResponseRootObject> {
    ...
    const { body } = await this.client.request.send<MediaRepositoryConfigureResponseRootObject>({
      ...
      method: 'POST',
      form: {
        source_type: options.source_type || 'library',
        caption: options.caption,
        upload_id: options.upload_id,
        disable_comments: options.disable_comments ? '1' : '0',
        like_and_view_counts_disabled: options.likeAndViewCountsDisabled ? '1' : '0',
        igtv_share_preview_to_feed: options.igtvSharePreviewToFeed ? '1' : '0',
        is_unified_video: '1',
    ...
    await this.client.request.sendUnauthenticatedOptionsRequest(
      {
        ...
        headers: {
          'access-control-request-headers':
            'content-type,x-asbd-id,x-csrftoken,x-ig-app-id,x-ig-www-claim,x-instagram-ajax',
          'access-control-request-method': 'POST',
        },
      },
      true
  ...
  public async storyQuestionResponse(
    ...
    if (typeof options.response === 'undefined') {
      options = defaultsDeep(options, {
        music_browse_session_id: chance.guid({ version: 4 }),
      })
    }
