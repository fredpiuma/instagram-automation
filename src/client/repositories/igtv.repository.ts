...
export class IgtvRepository extends Repository {
  public async writeSeenState(
    options: IgtvWriteSeenStateOptions
  ): Promise<StatusResponse> {
    const { body } = await this.client.request.send({
      url: '/api/v1/igtv/write_seen_state/',
      method: 'POST',
      form: this.client.request.sign({
        seen_state: JSON.stringify(
          defaults(options, { impressions: {}, grid_impressions: [] })
        ),
  ...
  public async search(
    query: string = ''
  ): Promise<IgtvSearchResponseRootObject> {
    const { body } =
      await this.client.request.send<IgtvSearchResponseRootObject>({
        // this is the same method in the app
        url: `/api/v1/igtv/${
          query && query.length > 0 ? 'search' : 'suggested_searches'
        }/`,
        method: 'GET',
        qs: {
          query
