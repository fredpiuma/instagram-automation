...
export class UploadRepository extends Repository {
  ...
  public async photo(options: UploadPhotoOptions): Promise<UploadRepositoryPhotoResponseRootObject> {
    ...
    const name = `fb_uploader_${uploadId}${random(100000, 999999)}`
    ...
    const { body } = await this.client.request.send<UploadRepositoryPhotoResponseRootObject>({
      ...
      headers: {
        ...
        offset: 0,
        'x-instagram-rupload-params': JSON.stringify(UploadRepository.createPhotoRuploadPhotoParams(options, uploadId)),
        'x-entity-name': name,
  ...
  private static createPhotoRuploadPhotoParams(options: UploadPhotoOptions, uploadId: number | string) {
    return {
      media_type: '1',
      upload_id: uploadId.toString(),
      upload_media_height: options.height,
      upload_media_width: options.width,
    }
  }
