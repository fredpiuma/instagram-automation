...
export class PublishService extends Repository {
  ...
  private static makeLocationOptions(location?: PostingLocation): any {
    ...
    if (typeof location !== 'undefined') {
      const { lat, lng, external_id_source: externalIdSource, external_id: externalId, name, address } = location
      options.location = {
        name,
        lat,
        lng,
      ...
      options.location[externalIdSource + '_id'] = externalId
  ...
  private async segmentedVideo(options: UploadSegmentedVideoOptions): Promise<
    StatusResponse & {
  ...
  > {
    ...
    const end = await this.client.upload.endSegmentedVideo({
      ruploadParams,
      streamId,
    })
