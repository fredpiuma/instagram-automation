import { IgApiClientBrowser } from '..'
    ...
    return ({ buffer }) => {
      ...
      for (let i = 0; i < numSections - 1; i++) {
        sections.push(
          buffer.slice(
            i * sectionSize,
            Math.min((i + 1) * sectionSize, buffer.byteLength)
          )
        )
      }
  ...
  sectionSize(sectionSize): SegmentDivider {
    return ({ buffer }) => {
      ...
      for (let i = 0; i < buffer.byteLength;) {
        const section = buffer.slice(
          i,
          Math.min(i + sectionSize, buffer.byteLength)
        )
